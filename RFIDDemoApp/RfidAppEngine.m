...
//#define ZT_NOTIFICATION_KEY_BARCODE         @"MotDcsNotificationKeyBarcode"
    ...
    dispatch_async(dispatch_get_main_queue(),
                   ^{
                       UIAlertView *alert = [[UIAlertView alloc]
                                             initWithTitle:ZT_RFID_APP_NAME
                                             message:message
                                             delegate:nil
    ...
    for (id<zt_IRfidAppEngineTriggerEventDelegate> delegate in m_TriggerEventDelegates )
    {
        if (delegate != nil && ([[[zt_RfidAppEngine sharedAppEngine] activeReader] getBatchModeRepeat] == NO))
        {
            dispatch_async([m_RadioOperationEngine getQueue], ^{
                dispatch_async(dispatch_get_main_queue(), ^{
                    [delegate onNewTriggerEvent:(SRFID_TRIGGEREVENT_PRESSED == triggerEvent)];
                });
            });
        }
    }
...
{
    srfidReaderVersionInfo *info = [[srfidReaderVersionInfo alloc] init];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (1 == buttonIndex)
    {
        ...
        [m_AppConfiguration setConfigAsciiPassword:[[alertView textFieldAtIndex:0] text] forReader:[m_ActiveReader getReaderID]];
        [self establishAsciiConnection];
    }
    else if (0 == buttonIndex)
    {
        /* cancel button pressed -> disconnect */
...
- (void) establishAsciiConnection
{
    ...
    else if (_conn_res == SRFID_RESULT_WRONG_ASCII_PASSWORD)
    {
        ...
        UIAlertView * alert = [[UIAlertView alloc] initWithTitle:ZT_RFID_APP_NAME message:@"Connection password required!" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Connect", nil];
